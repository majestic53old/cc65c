...
namespace cc65c {
	...
	namespace assembler {
		...
		static const std::string TOKEN_STR[] = {
			"BEGIN", "END", "IDENTIFIER", "KEYWORD_COMMAND", "KEYWORD_CONDITION", 
			"KEYWORD_DEFINE", "KEYWORD_INCLUDE", "KEYWORD_MACRO", "KEYWORD_REGISTER", 
			"LABEL", "LITERAL", "OPERATOR_BINARY", "OPERATOR_UNARY", "SCALAR", 
			"SYMBOL_ARITHMETIC", "SYMBOL_BRACE", "SYMBOL_BRACKET", "SYMBOL_IMMEDIATE", 
			"SYMBOL_POSITION", "SYMBOL_SEPERATOR",
			};

		#define TOKEN_STRING(_TYPE_) \
			((_TYPE_) > TOKEN_MAX ? EXCEPTION_UNKNOWN : STRING_CHECK(TOKEN_STR[_TYPE_]))
